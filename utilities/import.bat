...
for %%a in (import_these\*) do (
	...
	if !cftype!=="mp3" (
		...
		echo To import the voice clip, select the Import text-to-speech voice and type anything.
...
:voice
	pushd import_these
	if !cftype!=="voice" (
	ren "!cfid!" "rewritable.mp3"
	if not exist voice ( md voice )
	move /y "rewritable.mp3" voice\"rewritable.mp3" >nul
	echo Done.
	goto end
)
